...
public:
    ...
    {
        Matrix stateTransition = Matrix::Identity();
        stateTransition(iX, iV) = dt;

        const Vector newX = stateTransition * m_mean;

        Vector G;
        G(iX) = 0.5 * dt * dt;
        G(iV) = dt;

        const Matrix newP = stateTransition * m_cov * stateTransition.transpose() + G * G.transpose() * m_accelVariance;
